...
type BonusTransaction struct {
	...
	Created           string        `json:"created"`           // Время создания бонусной операции
	...
	Moment            string        `json:"moment"`            // Время проведения бонусной операции
	Agent             *Counterparty `json:"agent,omitempty"`   // Метаданные Контрагента	(Необходимое при создании)
	ParentDocument    Meta          `json:"parentDocument"`    // Метаданные связанного документа
	...
	ExecutionDate     string        `json:"executionDate"`     //Дата начисления бонусной операции
