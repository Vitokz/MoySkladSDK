...
type Employee struct {
	...
	Attributes   []Attribute `json:"attributes,omitempty"`   // Дополнительные поля Сотрудника
...
type Organization struct {
	...
	CertificateNumber      Meta          `json:"certificate_number"`               // Номер свидетельства
...
type Sign struct {
	...
	Updated   string `json:"updated"`   // Время загрузки файла на сервер
	Miniature Meta   `json:"miniature"` // Метданые миниатюры изображения
}
